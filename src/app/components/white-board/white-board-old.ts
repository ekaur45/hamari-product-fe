...
export class WhiteBoard implements AfterViewInit, OnDestroy {
    ...
    templates = signal<WhiteboardTemplate[]>([]);
     ...
     private selectionHeight = 0;
     private clipboardImageData: ImageData | null = null;
    ...
    onToggleGridVisible(): void {
        ...
        // Ensure canvas is properly sized before drawing grid
        if (this.gridCanvas && this.gridCanvas.nativeElement.width === 0) {
            // Canvas not initialized yet, trigger resize
            this.resizeCanvas();
        } else {
            this.drawGrid();
        }
     ...
     onCanvasMouseDown(event: MouseEvent | TouchEvent): void {
        ...
        if (tool === 'select') {
            ...
            if (this.hasSelection() && 
                pos.x >= this.selectionX && pos.x <= this.selectionX + this.selectionWidth &&
                pos.y >= this.selectionY && pos.y <= this.selectionY + this.selectionHeight) {
    ...
    onCanvasMouseMove(event: MouseEvent | TouchEvent): void {
        ...
        if (tool === 'select') {
            ...
            } else if (this.isMovingSelection && this.hasSelection()) {
                // Move selection
    ...
    private redrawCanvas(): void {
        ...
        if (this.history.length > 0 && this.historyStep >= 0) {
            ...
            img.onload = () => {
                ...
                if (this.hasSelection() && this.selectedImageData) {
                    this.canvasContext!.putImageData(this.selectedImageData, this.selectionX, this.selectionY);
                    this.drawSelectionRectangle();
                }
            };
    ...
    private drawSelectionRectangle(): void {
        ...
        if (this.isSelecting) {
            ...
            // Redraw canvas first to clear previous selection rectangle
            this.redrawCanvasWithoutSelection();
        } else if (this.hasSelection()) {
            // Drawing existing selection
            x = this.selectionX;
            y = this.selectionY;
        ...
        this.canvasContext.strokeStyle = '#3b82f6';
        this.canvasContext.lineWidth = 2;
        this.canvasContext.setLineDash([5, 5]);
        this.canvasContext.strokeRect(x, y, width, height);
        
        // Draw semi-transparent overlay
        this.canvasContext.fillStyle = 'rgba(59, 130, 246, 0.1)';
        this.canvasContext.fillRect(x, y, width, height);
        
        this.canvasContext.setLineDash([]);
    }
    
    private redrawCanvasWithoutSelection(): void {
        if (!this.canvasContext || !this.whiteboardCanvas) return;
    ...
    private finalizeSelection(): void {
        ...
        this.selectionX = Math.max(0, Math.min(x, canvas.width));
        this.selectionY = Math.max(0, Math.min(y, canvas.height));
        this.selectionWidth = Math.min(width, canvas.width - this.selectionX);
        this.selectionHeight = Math.min(height, canvas.height - this.selectionY);
        
        // Redraw canvas without selection rectangle to get clean base
        this.redrawCanvasWithoutSelection();
        
        // Extract selected image data from base canvas
        try {
            this.selectedImageData = this.canvasContext.getImageData(
                this.selectionX, 
                this.selectionY, 
                this.selectionWidth, 
    ...
    private moveSelection(deltaX: number, deltaY: number): void {
        if (!this.canvasContext || !this.whiteboardCanvas || !this.selectedImageData) return;
        
        const canvas = this.whiteboardCanvas.nativeElement;
        const newX = Math.max(0, Math.min(this.selectionX + deltaX, canvas.width - this.selectionWidth));
        const newY = Math.max(0, Math.min(this.selectionY + deltaY, canvas.height - this.selectionHeight));
        
        // Redraw canvas without selection to restore background
        this.redrawCanvasWithoutSelection();
        
        // Draw selection at new position
        this.canvasContext.putImageData(this.selectedImageData, newX, newY);
    ...
    onCopySelection(): void {
        if (this.selectedImageData) {
            this.clipboardImageData = this.selectedImageData;
            this.hasClipboard.set(true);
        }
    }
    
    onCutSelection(): void {
        if (this.selectedImageData && this.canvasContext) {
            this.clipboardImageData = this.selectedImageData;
            this.hasClipboard.set(true);
            
            // Clear selected area
            this.canvasContext.clearRect(this.selectionX, this.selectionY, this.selectionWidth, this.selectionHeight);
            this.saveState();
            this.hasSelection.set(false);
            this.selectedImageData = null;
            this.redrawCanvas();
    ...
    onPasteSelection(): void {
        if (this.clipboardImageData && this.canvasContext && this.whiteboardCanvas) {
            const canvas = this.whiteboardCanvas.nativeElement;
            // Paste at center of canvas
            const pasteX = (canvas.width - this.clipboardImageData.width) / 2;
            const pasteY = (canvas.height - this.clipboardImageData.height) / 2;
            
            this.canvasContext.putImageData(this.clipboardImageData, pasteX, pasteY);
            
            // Update selection to pasted area
            this.selectedImageData = this.clipboardImageData;
            this.selectionX = pasteX;
            this.selectionY = pasteY;
            this.selectionWidth = this.clipboardImageData.width;
            this.selectionHeight = this.clipboardImageData.height;
            this.hasSelection.set(true);
            this.saveState();
            this.drawSelectionRectangle();
        }
    ...
    onAlignSelection(alignment: 'left' | 'center' | 'right' | 'top' | 'bottom'): void {
        if (!this.hasSelection() || !this.selectedImageData || !this.canvasContext || !this.whiteboardCanvas) return;
        ...
        switch (alignment) {
            case 'left':
                ...
                break;
            case 'center':
                newX = (canvas.width - this.selectionWidth) / 2;
                break;
            case 'right':
                newX = canvas.width - this.selectionWidth;
                break;
            case 'top':
                newY = 0;
                break;
            case 'bottom':
                newY = canvas.height - this.selectionHeight;
                break;
        }
    ...
    onLayerChange(direction: 'front' | 'back'): void {
        // For a simple canvas, layer changes would require object tracking
        // This is a placeholder for future implementation
        console.log('Layer change:', direction);
    }
    
    onDeleteSelection(): void {
        if (this.hasSelection() && this.canvasContext) {
            this.canvasContext.clearRect(this.selectionX, this.selectionY, this.selectionWidth, this.selectionHeight);
            this.saveState();
            this.hasSelection.set(false);
            this.selectedImageData = null;
